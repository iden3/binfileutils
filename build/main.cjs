...
async function readBinFile(fileName, type, maxVersion, cacheSize, pageSize) {
	...
	if (await fd.readULE32() > maxVersion) throw new Error("Version not supported");
...
async function startWriteSection(fd, idSection) {
	...
	fd.writingSection = { pSectionSize: fd.pos };
...
async function copySection(fdFrom, sections, fdTo, sectionId, size) {
	if (typeof size === "undefined") size = sections[sectionId][0].size;
